...
def group_and_distribute_sum(array):
    ...
    group_sums = jax.ops.segment_sum(
        array, group_indices, num_segments=array.shape[0] // group_size
    )
...
def make_train(config):
    ...
    if config["ENV_KWARGS"].get("interest", False):
        s_interest_schedule = calculate_s_interest_schedule(
            config["ENV_KWARGS"]["num_agents"]
        )
        config["ENV_KWARGS"]["s_interest_schedule"] = s_interest_schedule
        config["ENV_KWARGS"]["s_interest_change_every"] = config.get(
            "S_INTEREST_CHANGE_EVERY", 30000000
        )
        print(f"Using s_interest schedule: {s_interest_schedule}")
        print(
            f"Changing every {config['ENV_KWARGS']['s_interest_change_every']} timesteps"
        )
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            metric["eat_own_coins"] = (
                metric["eat_own_coins"] * config["ENV_KWARGS"]["num_inner_steps"]
            )
...
SINGLE_RUN_KWARGS = {"wandb_name": "svo_cnn_coins", "group_name": "coins"}
